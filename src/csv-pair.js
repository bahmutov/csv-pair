...
function csvPair(filenames, outputFilename) {
  la(check.arrayOf(check.unemptyString, filenames), 'expected list of input filenames', filenames);
  la(check.unemptyString(outputFilename), 'expected output filename', outputFilename);
  ...
  */
  ...
  fs.writeFileSync(outputFilename, csvText);
  console.log('saved combined CSV', outputFilename);
}

module.exports = csvPair;
